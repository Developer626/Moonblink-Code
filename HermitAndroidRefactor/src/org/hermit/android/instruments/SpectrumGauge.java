    ...
    extends Gauge
{
    ...
    public void switchView() {
    	logFreqScale = !logFreqScale;
 
        // Redraw background to show the new frequency scale.
        drawBg(bgCanvas, getPaint());
    }
    ...
    private void drawBg(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i <= 10; i += step) {
            ...
            if (logFreqScale) {
            	// Calculate the frequency corresponding to a line
           		f =(int) Math.pow(2, (log2(rf)*i/10 ) );
            } else {
            	f = nyquistFreq * i / 10;
        	}
    ...
    private void logGraph(float[] data, Canvas canvas, Paint paint) {
        ...
        for (int i = 1; i < len; ++i) {
            ...
            // For freq f, calculate x.
       		final float x = (float) ( spectGraphX + spectGraphWidth*log2(f)/log2(rf) );
	...
	private void linearGraph(float[] data, Canvas canvas, Paint paint) {
        ...
        for (int i = 1; i < len; ++i) {
            ...
            // Draw the bar.
            float x = spectGraphX + 1;
            if (logFreqScale) {
                // For freq f, calculate x.
           		x += (float) (spectGraphWidth*log2(i*lf)/log2(rf) );
            } else {
            	x += i * bw;
            }
    ...
    private boolean logFreqScale = false;
