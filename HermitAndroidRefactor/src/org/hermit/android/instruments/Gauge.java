...
public class Gauge
{
	...
	/**
	 * Determine best text height to fit in given dimensions. 
	 */
	protected float findTextSize(float w, float h, String template, Paint paint) {
        float size = h;
        do {
	...
	protected class Area {
		...
		protected float width() { return right - left; }
		protected float height() { return bottom - top; }
	}
	...
	protected class TextArea extends Area {
		...
		public TextArea(Area area, String sample_text, Paint paint) {
			super( area ); 
			
	        text_size = findTextSize( width(), height(), sample_text, paint);
		}

		public TextArea(float left, float top, float width, float height, String sample_text, Paint paint) {
			super( left, top, width, height); 
			
	        text_size = findTextSize( width(), height(), sample_text, paint);
		}
		
		public void draw( String text, Canvas canvas, Paint paint) {
	        paint.setTextSize( text_size );
	        float pw = paint.measureText( text );

	        final float px = left + ( width() - pw )/2;
	        
	        // Apparently, the coordinate of a text is the bottom left corner of the text.
	        final float py = top + ( height() + text_size )/2 - paint.descent();
	        canvas.drawText( text.toCharArray(), 0, text.length(), px, py, paint);
		}
	}
	...
	public void setSampleRate(int rate) { /* Do nothing */ }
	
	// Following set defaults to false and do nothing.
	//Override in the derived classes to request and handle specific data
    public boolean needWaveForm() { return false; }
    
    public void setWaveForm(short[] buffer, int off, int len, float bias, float range) { 
    	// Do nothing
    }
    
    public boolean needPowerDb() { return false; }
    
    public void setPowerDb( double currentPower ) {
    	// Do nothing
    }
    
    public boolean needFFT() { return false; }
